/*********************************************************************
  ...
  Last updated:  October 10, 2015 - Hunter Johnson
...
{
    int position = hashing(key(wordc));
    
    if(!find(wordc, key(wordc), position))
        insert(wordc, position);
...
{
    int position = hashing(s);
    if(!find(wordc, s, position))
        return false;
    else
        return true;  // find() method already sets wordc = storage[position].object 
...
void HashTable::statistic()
{
    ...
    {
        for(int i=0; i < capacity; i++)
            cout << storage.size() << " objects at index " << i << endl;
    }
} // HashTable::statistic()
