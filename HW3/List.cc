/*********************************************************************
  ...
  File name:     List.cc
  Last updated:  September 15, 2015 - Hunter Johnson
...
bool List::isOrdered()
{
	...
	while (iter2)
	{
	    if (*iter1 <= *iter2)   // check if values are in order
		{
			++iter1;
			++iter2;
		}
...
bool List::removeFront()
{
  ...
  ListNodePtr n = head; // front node to be removed
  
  --size; // decrement size  (memory deallocation)
